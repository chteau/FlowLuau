...
export interface LessThanOrEqualNodeData {
    mode?: "linear" | "expression";
    expression?: string;
}

export type LessThanOrEqualNodeProps = NodeProps & { data: LessThanOrEqualNodeData };

/**
 * Comparison node that checks less than or equal (<=).
 ...
 */
const LessThanOrEqualNode = memo(({ data, selected }: LessThanOrEqualNodeProps) => {
    ...
    const updateData = useCallback(
        (partial: Partial<LessThanOrEqualNodeData>) => {
            setNodes((nodes) =>
                nodes.map((n) =>
    ...
    return (
        <NodeTemplate
            details={{
                ...
                icon: mode === "expression" && !isValid ? AlertTriangle : ChevronLeft,
                ...
                {mode === "expression" && (
                    <div className="space-y-1">
                        <Input
                            ...
                            placeholder="e.g., level <= maxLevel"
...
LessThanOrEqualNode.displayName = "LessThanOrEqualNode";

(LessThanOrEqualNode as any).getHandles = (data: LessThanOrEqualNodeData) => ({
    inputs:
...
export default LessThanOrEqualNode;