...
export interface LessThanNodeData {
    mode?: "linear" | "expression";
    expression?: string;
}

export type LessThanNodeProps = NodeProps & { data: LessThanNodeData };

/**
 * Comparison node that checks less than (<).
 ...
 */
const LessThanNode = memo(({ data, selected }: LessThanNodeProps) => {
    ...
    const updateData = useCallback(
        (partial: Partial<LessThanNodeData>) => {
            setNodes((nodes) =>
                nodes.map((n) =>
...
LessThanNode.displayName = "LessThanNode";

(LessThanNode as any).getHandles = (data: LessThanNodeData) => ({
    inputs:
