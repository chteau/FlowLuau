...
import { Ampersand as AndIcon, AlertTriangle } from "lucide-react";
...
export type AndNodeProps = NodeProps & { data: AndNodeData };

/**
 * Logical AND node that performs boolean AND operation.
 * Linear mode exposes two Boolean inputs (A AND B); expression mode evaluates a custom logical expression.
 * Outputs a single Boolean-typed result.
 */
const AndNode = memo(({ data, selected }: AndNodeProps) => {
    ...
    const handleExpressionChange = useCallback(
        (e: React.ChangeEvent<HTMLInputElement>) => {
            ...
            const valid =
                expr.trim() === "" ||
                /^\s*(?:[a-zA-Z_]\w*|true|false)\s*(?:and|or|not)\s*.+$/i.test(expr);
            setIsValid(valid);
        },
        [updateData]
    );
    ...
    return (
        <NodeTemplate
            details={{
                ...
                icon: mode === "expression" && !isValid ? AlertTriangle : AndIcon,
                name: "And",
                description:
                    mode === "linear"
                        ? "Logical AND (A and B)"
                        : isValid
                        ? `Evaluates: ${expression || "A and B"}`
                ...
                {mode === "expression" && (
                    <div className="space-y-1">
                        <Input
                            ...
                            placeholder="e.g., isReady and hasPermission"
...
(AndNode as any).getHandles = (data: AndNodeData) => ({
    inputs:
